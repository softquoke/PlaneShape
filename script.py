...
class PlaneShape(Frame):    
    ...
    def selection_of_connecting_lines(self):        
        ...
        for i in range(0, len(self.points_figure) - 2):
            for j in range(i + 2, len(self.points_figure) - 1):
                # Получение уравнения прямой по ее двум точкам
                ...
                for basic_line in self.basic_lines:
                    ...
                    # Проверка на совпадение с базовой линией фигуры
                    # Данную провеку обязательно должны проходить соединяющие линии
    ...
    def is_connecting_line(self, line, basic_line) -> bool:
        self.is_intersection_lines(line, basic_line)
        return True

    def is_intersection_lines(self, line_one, line_two) -> dict:
    ...
    def draw_connecting_lines(self):
        ...
        for i in range(0, len(self.connecting_lines)):
            self.draw_line(self.connecting_lines[i]["coordinates"], width=3, fill="#faa7c3")
